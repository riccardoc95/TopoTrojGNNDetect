...
class ReverseLambdaPattern:
    ...
    def get_data(self) -> torch.tensor:
        ...
        """
        return torch.from_numpy(np.transpose(self.pattern, (2, 0, 1)))
    ...
    def get_mask(self) -> torch.tensor:
        ...
        """
        return torch.from_numpy(self.mask).unsqueeze(0).repeat(self.num_chan, 1, 1)
...
if __name__ == '__main__':
    trigger = ReverseLambdaPattern(num_rows=28, num_cols=28, num_chan=1, trigger_cval=255,
                 bg_cval=0, thickness=1, pattern_style='graffiti',
                 dtype=np.uint8)

    print(trigger.get_data().shape)
    print(trigger.get_mask().shape)